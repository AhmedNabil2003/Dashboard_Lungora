...
export const getAllArticles = async () => {
  try {
    ...
    if (
      res.data?.isSuccess &&
      res.data.result &&
      Array.isArray(res.data.result?.article)
    ) {
...
export const getArticleById = async (id) => {
  try {
    ...
    throw new Error(
      `Something went wrong while fetching article with ID ${id}. Please try again later.`
    );
  }
};
...
export const getArticlesByCategoryId = async (categoryId) => {
  try {
    const res = await axiosInstance.get(
      `/Article/GetArticlesByCategoryId/${categoryId}`
    );
    if (res.data?.isSuccess && res.data.result) {
      return {
        articles: res.data.result.articles || [],
        numberOfArticles:
          res.data.result.numerOfArticles ||
          res.data.result.numberOfArticles ||
          0, // Handle typo
      };
    }
    ...
    throw new Error(
      `Something went wrong while fetching articles for category ID ${categoryId}. Please try again later.`
    );
  }
};
...
export const createArticle = async (data) => {
  try {
    ...
    const res = await axiosInstance.post("/Article/CreateArticle", data, {
      headers,
    });
    ...
    throw new Error(
      error.message ||
        "Something went wrong while creating the article. Please try again later."
    );
  }
};
...
export const updateArticle = async (id, data) => {
  try {
    ...
    const res = await axiosInstance.put(`/Article/EditArticle/${id}`, payload, {
      headers,
    });
    if (res.data?.isSuccess) {
      return res.data;
    ...
    throw new Error(
      error.message ||
        `Something went wrong while updating article with ID ${id}. Please try again later.`
    );
  }
};
...
export const deleteArticle = async (id) => {
  try {
    ...
    throw new Error(
      error.message ||
        `Something went wrong while deleting article with ID ${id}. Please try again later.`
    );
  }
};
